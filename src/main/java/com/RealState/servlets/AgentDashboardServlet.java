...
public class AgentDashboardServlet extends HttpServlet {
    ...
    private static final String CURRENT_DATETIME = "2025-03-26 20:13:11";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // Set current date/time and user info
            request.setAttribute("currentDateTime", CURRENT_DATETIME);
    ...
    private void markMessageAsRead(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (int i = 0; i < messages.size(); i++) {
                ...
                if (message.get("messageId").getAsString().equals(messageId)) {
    ...
    private void markAllMessagesAsRead(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (int i = 0; i < messages.size(); i++) {
                JsonObject message = messages.get(i).getAsJsonObject();
                if (message.get("agentId").getAsString().equals(agentId) && 
                    !message.get("read").getAsBoolean()) {
                    message.addProperty("read", true);
                    updated = true;
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < messages.size(); i++) {
                ...
                if (message.get("messageId").getAsString().equals(messageId)) {
                    ...
                    message.addProperty("read", true);
                    message.addProperty("status", "REPLIED");
                    
                    // Add reply information
                    JsonObject reply = new JsonObject();
                    reply.addProperty("replyText", replyText);
                    reply.addProperty("replyTimestamp", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                    reply.addProperty("replyAgentId", agentId);
                    
                    // Add reply to message
                    message.add("reply", reply);
    ...
    private List<Map<String, Object>> loadAgentMessages(String agentId) throws IOException {
        ...
        JsonArray allMessages = readMessagesFromFile();
        List<Map<String, Object>> agentMessages = new ArrayList<>();
        
        // Filter messages for this agent
        for (int i = 0; i < allMessages.size(); i++) {
            JsonObject jsonMessage = allMessages.get(i).getAsJsonObject();
            if (jsonMessage.get("agentId").getAsString().equals(agentId)) {
                // Convert JsonObject to Map
                Type type = new TypeToken<Map<String, Object>>(){}.getType();
                Map<String, Object> message = gson.fromJson(jsonMessage, type);
     ...
     */
    private JsonArray readMessagesFromFile() throws IOException {
        // Get file path
        ...
        if (!file.exists()) {
            ...
            writeMessagesToFile(emptyArray);
            return emptyArray;
        }
     ...
     */
    private void writeMessagesToFile(JsonArray messages) throws IOException {
        // Get file path
        Path filePath = Paths.get(MESSAGES_FILE_PATH);
